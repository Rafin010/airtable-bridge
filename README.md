...
A practical Make custom app starter for **Airtable records and bases**. This repository is designed for import and continued development in the Make Apps Editor or Apps SDK workflow.

## What it provides

The app includes a reusable basic-token connection, a **Get resource** action, a **Search resources** module, a **Make an API call** universal module, and an RPC starter for dynamic resource loading. The endpoint defaults to `https://api.airtable.com/v0` and can be overridden in the connection settings when the service uses a tenant-specific host.
...
This is an independent community starter and is not affiliated with or endorsed by Airtable Bridge. Review the service API terms, authentication requirements, rate limits, and licensing before publishing.
